...
> ⚠️ **주의할 점**
> Query, Key 등이 같다는 것은 **벡터의 값이 같다는 것이 아니라 벡터의 출처가 같다**는 뜻
...
> 💡 **왜 cross 인가? ⇒ 출력 개수를 고정하기 위해**
>
> **규칙 : 출력 개수 = 항상 Q의 개수**
>
> - **self** 로 하면
>   - in 5개 → out 5개
>   - 관측이 늘면 출력도 늘어남 → 무엇이 액션인지 정할 수 없음
> - **cross** 로 하면
>   - Q 10개 (고정) / K, V 관측 몇 개든 → out 항상 10개
> - self + 학습 가능 쿼리 토큰을 앞에 붙이면 흉내는 가능 (ViT의 CLS 방식)
>   - 그래도 cross를 쓰는 이유
>     - 의도가 구조에 드러남
>     - 연산이 적음 : `(10+5)² = 225` vs `10×5 = 50`
>
> ex) SUREFlow MGAD
> Q = 액션 쿼리 10개 / K, V = 멀티모달 토큰 → out 10개 → 7차원 액션
...
> **⇒ 경량화 연결 : head pruning**
> "공짜로 다양성을 얻는다"고 했지만, 얻은 다양성이 다 쓸모 있진 않음.
> 학습된 헤드 중 일부는 중복이라 제거해도 성능이 거의 유지된다는 연구 계열 존재
> (차후에 다룰 주제)
