...
x_train = torch.tensor([1.0, 2.0, 3.0]) # Dummy Data
...
⇒ x_train 입력값을 받아서, 초기값이 0인 가중치와 편향으로 선형 예측값을 계산
...
x_train = torch.FloatTensor([[1],[2],[3]]) # 입력 데이터

y_train = torch.FloatTensor([[1],[2],[3]]) # 정답 레이블
...
for epoch in range(epochs + 1) :
    ...
    hypothesis = x_train * W # 선형회귀 : 모델 상태 수치



    cost = torch.mean((hypothesis - y_train) ** 2) # cost_function : 오차 범위 평가 

    gradient = torch.sum((W * x_train - y_train) * x_train) # 경사하강법 : 최적화
...
W = torch.zeros(1, requires_grad=True) # 추가
...
- 경사하강법 담당
    ...
    - optimizer이 처리함
        ...
        optimizer.zero_grad() # 1. 기존 기울기 초기화
